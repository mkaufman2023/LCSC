...
class ProductDetails:
    ...
    def __post_init__(self):
        ...
        # ladders = list(price_details.keys())
        # details = list(price_details.values())
        # for i in range(1, len(price_details)):
        #     delta_ladder = ladders[i] - ladders[i-1]
        #     delta_discount_pct = details[i].discount_pct - details[i-1].discount_pct
        #     slope = delta_discount_pct / delta_ladder
        #     slopes.append((ladders[i], slope))
        # if len(slopes) > 0:
        #     slopes.sort(key=lambda x: x[1], reverse=True)
        #     optimal_order_qty = slopes[0][0]
        #     print(optimal_order_qty, slopes)
        # super().__setattr__("optimal_order_quantity", optimal_order_qty)
...
class SearchResults:
    ...
    def view(self):
        view_dict([x.as_dict() for x in self.results])
